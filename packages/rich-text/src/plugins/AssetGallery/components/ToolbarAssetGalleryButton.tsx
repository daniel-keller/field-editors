...
import { addAssetGallery } from '../addAssetGallery';
import { BLOCKS } from '../../../rich-text-types/src';

export interface ToolbarAssetGalleryButtonProps {
  isDisabled: boolean | undefined;
}

export function ToolbarAssetGalleryButton(props: ToolbarAssetGalleryButtonProps) {
  const editor = useContentfulEditor();

  async function handleClick() {
    if (!editor) return;
    editor.tracking.onToolbarAction('insertAssetGallery');

    if (editor.selection) {
      addAssetGallery(editor);
    }
    focus(editor);
  }
